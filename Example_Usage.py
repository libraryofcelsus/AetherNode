...
async def generate_and_retrieve_text(data, host="http://127.0.0.1:8000"):
    async with aiohttp.ClientSession() as session:
        ...
        async with session.post(f"{host}/generate-text/", json=data) as post_response:
            if post_response.status == 200:
                ...
                # If there's an error with the POST request, return the error
...
if __name__ == "__main__":
    while True:
        ...
        result = asyncio.run(generate_and_retrieve_text(data))
        print(result)
